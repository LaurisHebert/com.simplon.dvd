    ...
    getAllDvds = async () => {
        try {
            ...
            const dvds: Dvd[] = response.data.map((data: any) => ({
                id: data.id, directedBy: data.directedBy, duration: data.duration, filmCover: data.filmCover, genre: data.genre, isan: data.isan, quantity: data.quantity, releaseDate: new Date(data.releaseDate), resume: data.resume, title: data.title, type: data.type
            }))
    ...
    createDvd = async (dvd: Dvd) => {
        try {
            const dvdToSent: DvdToSent = {
                id: dvd.id, directedBy: dvd.directedBy, duration: dvd.duration, filmCover: dvd.filmCover, genre: dvd.genre, isan: dvd.isan, quantity: dvd.quantity, releaseDate: dvd.releaseDate.toLocaleDateString('fr-FR', {day: '2-digit', month: '2-digit', year: 'numeric'}), resume: dvd.resume, title: dvd.title, type: dvd.type
            }
